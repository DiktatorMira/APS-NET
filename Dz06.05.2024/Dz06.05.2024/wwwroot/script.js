$(function () {
    const urow = function (user) {
        return "<tr data-rowid='" + user.id + "'><td>" + user.id + "</td>" +
            "<td>" + user.login + "</td> <td>" + user.fullname + "</td>" +
            "<td>" + user.password + "</td>" +
    ...
    function CreateUser(userLogin, userFullname, userPassword) {
        $.ajax({
            url: "https://localhost:7005/api/users",
            contentType: "application/json",
            method: "POST",
    ...
    function EditUser(userId, userLogin, userFullname, userPassword) {
    ...
    $("#submit1").click(function (e) {
        ...
        let fullname = document.forms["UserForm"].elements["fullname"].value;
        let password = document.forms["UserForm"].elements["password"].value;
        if (id == 0) CreateUser(login, fullname, password);
        else EditUser(id, login, fullname, password);
    });
    ...
    function CreatePerformer(performerFullname) {
        $.ajax({
            url: "https://localhost:7005/api/performers",
            contentType: "application/json",
            method: "POST",
    ...
    function EditPerformer(performerId, performerFullname) {
        let request = JSON.stringify({
            id: performerId,
            fullname: performerFullname
        });
    ...
    $("#submit3").click(function (e) {
        ...
        let fullname = document.forms["PerformerForm"].elements["pfullname"].value;
        if (id == 0) CreatePerformer(fullname);
        else EditPerformer(id, fullname);
    });
    ...
    function GetUser(id) {
        $.ajax({
            ...
            success: function (user) {
                ...
                document.forms["UserForm"].elements["fullname"].value = user.fullname;
    ...
    function GetPerformer(id) {
        $.ajax({
            ...
            success: function (performer) {
                ...
                document.forms["PerformerForm"].elements["pfullname"].value = performer.fullname;
            },
            error: function (jqXHR, exception) {
                alert(jqXHR.status + '\n' + exception);
            }
